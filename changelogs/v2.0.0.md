...
* Added support for standard getty implementations such as agetty(8) and the FreeBSD getty(8).
    ...
    * LeanInit will now use `unsigned char` variables by default when setting small variables. This can be changed to `unsigned int` for performance at the cost of RAM by passing `-DUINT32` in `CFLAGS`.
